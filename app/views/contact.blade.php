@extends('layouts.master')
@section('title')
<title>Contact Us</title>
@stop
@section('content')
    <div class="jumbotron">
        <h1 class="canevalee centeredText">Contact Us</h1>
        <p class="centeredText">
            Please feel free to contact us for more information on our dogges, you can call us at <a href="tel:+12105443004">210-544-3004</a>.
        </p>
    </div>
    <hr>
    <h1 class="canevalee centeredText">Make Deposits. Pay Online,</h1>
    <h1 class="canevalee centeredText"> It’s Easy and Secure With PayPal</h1>
    <div class="row">
        <a href=""><img src="/img/paypal.jpg" class="col-md-4 col-md-offset-4" alt="paypal"></a>
    </div>
    <hr>
    <div class="jumbotron">
    <div class="row">
        <img src="/img/mailDog.gif" class="col-md-2" alt="">
        <a href="mail:bullnanzabulldogges@gmail.com" class="col-md-8 col-md-offset-1"><h2>bullnanzabulldogges@gmail.com</h2></a>
    </div>
    <div class="row">
        <h2 class="centeredText"><u>Mailing Address</u></h2>
        <img class="col-md-4 col-sm-8 col-xs-8" src="/img/shasta.jpg" alt="">
        <h4 class="centeredText col-md-4 col-sm-8 col-xs-8">
            Bullnanza Bulldogges<br>
            P.O. Box 331<br>
            Balverde, Tx 78163
        </h4>
        <img class="col-md-3 col-md-offset-1 col-sm-8 col-xs-8" src="/img/bath.png" alt="">
    </div>
    </div>
    <hr>
    <h1 class="canevalee centeredText">
        Our Bulldogges Are Kid Tested and Mother Approved!
    </h1>
    <div class="row">
        <img class="col-md-4 col-sm-8 col-xs-8" src="/img/contactPage/1.jpg" alt="">
        <h4 class="centeredText col-md-4 col-sm-8 col-xs-8">
            Thanks for visiting our site!
        </h4>
        <img class="col-md-4 col-sm-8 col-xs-8" src="/img/contactPage/2.jpg" alt="">
    </div>
    <div class="row">
        <br>
        <img class="col-md-4 col-sm-8 col-xs-8" src="/img/contactPage/3.jpg" alt="">
        <img class="col-md-4 col-sm-8 col-xs-8" src="/img/contactPage/4.jpg" alt="">
        <img class="col-md-4 col-sm-8 col-xs-8" src="/img/contactPage/5.jpg" alt="">
    </div>

    <hr>
    <h2 class="centeredText">
        <a href="tel:+12105443004">210-544-3004</a>
    </h2>
    <h4 class="centeredText">
        If our bullies are what you’re looking for or if you would like more information, please contact us with any questions at
    </h4>
    <h2 class="centeredText">
        <a href="tel:+12105443004">210-544-3004</a>
    </h2>
@stop